...
func update_visual() -> void:
	...
	var vscroll := $MarginContainer/HBoxContainer/MapGenerator/ScrollContainer.get_v_scroll_bar() as VScrollBar
	await vscroll.changed # Await the vscroll to update its max.
	vscroll.value = 0.
	var scroll_tween := create_tween().set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_SINE)
	scroll_tween.tween_property(vscroll, "value", vscroll.max_value, 2.0)
